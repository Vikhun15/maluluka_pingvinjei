...
public class Proto {
    ...
    // A modell elemei
    private static final Map<String, Csomopont> csomopontok = new LinkedHashMap<>();
    private static final Map<String, Utszakasz> utszakaszok = new LinkedHashMap<>();
    private static final Map<String, Sav> savok = new LinkedHashMap<>();
    private static final Map<String, Jarmu> jarmuvek = new LinkedHashMap<>();
    ...
    private static void parancsRandom(String[] t) {
        if (t.length < 2) {
            System.out.println("ERROR: random on|off");
            return;
        }
    ...
    private static void parancsBuild(String[] t) {
        if (t.length < 3) {
            System.out.println("ERROR: build node|road|lane");
            return;
        }
        ...
        switch (t[1]) {
            case "node" -> {
                ...
                switch (tipus) {
                    case "Bolt" -> c.setEpulet(new Bolt());
                    case "Otthon" -> c.setEpulet(new Otthon());
                    case "Munkahely" -> c.setEpulet(new Munkahely());
                    case "Benzinkut" -> c.setEpulet(new Benzinkut());
                    default -> c = new Csomopont() {
                    };
                }
    ...
    private static void parancsCreate(String[] t) {
        if (t.length < 4) {
            System.out.println("ERROR: create <típus> <id> <sav_id>");
            return;
        }
    ...
    private static void parancsStat(String[] t) {
        if (t.length < 2) {
            System.out.println("ERROR: stat <id>|all");
            return;
        }
        if (t[1].equals("all")) {
