...
public class View {
    ...
    public void display(Palya palya) {
        ...
        for (Csomopont cs : palya.getCsomopontok()) {
            ...
            String vehicle = cs.getAktualisJarmu() != null ? cs.getAktualisJarmu().getClass().getSimpleName() + "(" + cs.getAktualisJarmu().id + ")" : "Nincs";
            System.out.println("  ID: " + cs.id + ", Típus: " + type + ", Jármű: " + vehicle + ", Pozíció: (" + cs.getX() + "," + cs.getY() + ")");
        }

        System.out.println("\nÚtszakaszok:");
        for (Utszakasz ut : palya.getUtszakaszok()) {
            System.out.println("  ID: " + ut.id + ", Kezdő: " + ut.kezdoCsomopont.id + ", Vég: " + ut.vegCsomopont.id + ", Típus: " + ut.getClass().getSimpleName() + ", Sávok: " + ut.getSavok().size());
            for (Sav s : ut.getSavok()) {
                System.out.println("    Sáv ID: " + s.id + ", Hó: " + s.gethoRetegek() + ", Jeges: " + s.jeges() + ", Sózva: " + s.getSozva());
            }
        }

        System.out.println("\nJárművek:");
        for (Jarmu j : palya.getJarmuvek()) {
            String pos = j.getAktualisSav() != null ? "Sáv a " + j.getAktualisSav().getUtszakasz().id + "-n" : "Nincs";
            System.out.println("  ID: " + j.id + ", Típus: " + j.getClass().getSimpleName() + ", Pozíció: " + pos);
            if ("Hokotro".equals(j.getClass().getSimpleName())) {
                ...
                Auto a = (Auto) j;
                String home = a.getOtthon() != null ? a.getOtthon().id + "" : "Nincs";
                String work = a.getMunkahely() != null ? a.getMunkahely().id + "" : "Nincs";
